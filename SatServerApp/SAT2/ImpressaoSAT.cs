...
namespace invoiceServerApp
{
    ...
    public class ImpressaoSAT
    {
        ...
        //        //sb.AppendLine(String.Format("<l></l><qrcode>{0}<lmodulo>4</lmodulo><correcao>1</correcao></qrcode><gui></gui><g><g>", "123456789ABCDEFGHIJKLMNOPQRSTUVXZ123456789"));
        ...
        private void DefineDadosImpressaoCompleto()
        {
            try
            {
                ...
                Linhas.Add(String.Format("<bc>{0}", dadosEmit.razao));
                Linhas.Add(String.Format("<nb>Endereço:{0}, No:{1}", dadosEmit.logradouro, dadosEmit.numero));
                Linhas.Add(String.Format("<nb>CNPJ:{0}     I.E.:{1}     I.M.: ", dadosEmit.CNPJ, dadosEmit.IE));

                Linhas.Add(String.Format("<bc>----------------------------------------------------"));

                Linhas.Add(String.Format("<bc>Extrato No. {0}", dadosSAT.numeroCFe));
                Linhas.Add(String.Format("<bc>CUPOM FISCAL ELETRONICO - SAT "));
                ...
                if (!String.IsNullOrEmpty(dadosCli.cpf_cnpj_destinatario))
                {
                    Linhas.Add(String.Format("<nn>CPF/CNPJ Consumidor:    {0}", dadosCli.cpf_cnpj_destinatario));
                }
                else
                {
                    Linhas.Add(String.Format("<nn>CPF/CNPJ Consumidor:    {0}", "Nao Identificado"));
                }
                ...
                foreach (ItensNotaSAT i in itens)
                {
                    ...
                    Linhas.Add(String.Format("<nc>{0,-6} {1,-20} {2,-3} {3,-6} {4,-6} {5,-6}"
                        , i.codigo
                        , tmpDesc
                        , i.quantidade
                        , i.unidade
                ...
                Linhas.Add(String.Format("<nl>"));

                Linhas.Add(String.Format("<nn> {0,-20}{1,34}"
                    , "Sub Total:"
                    , vt.ToString("F2")
                    ));
                ...
                foreach (PgtNotaSAT p in pgts)
                {
                    ...
                    Linhas.Add(String.Format("<bc> {0,-20}{1,34}"
                        , p.descPgt + " R$"
                        , p.valorPgt
                        ));
                }
                ...
                Linhas.Add(String.Format("<bc>Informacoes Adicionais"));

                if (dadosSAT.infCpl.Length > 0)
                {
                    String[] tmp = dadosSAT.infCpl.Split('\\');
                    foreach (string s in tmp)
                        Linhas.Add(String.Format("<nn>{0}", s));
                }
                Linhas.Add(String.Format("<bc>------------------------------------------------"));

                //Rodapé ---------------------------------------------------
                Linhas.Add(String.Format("<bc>SAT No. {0}", dadosSAT.numSerieSAT));
                Linhas.Add(String.Format("<nc>{0} - {1}", dadosSAT.dataEmis, dadosSAT.horaEmis));
                ...
                Linhas.Add(String.Format("<nc>{0}", chave));

                Linhas.Add(String.Format("<br>{0}", tmpChave.Substring(0, 22)));
                    
                Linhas.Add(String.Format("<br>{0}", tmpChave.Substring(22, 22)));

                //sb.AppendLine(String.Format("<qrcode>{0}<lmodulo>4</lmodulo><correcao>1</correcao></qrcode><gui></gui><g><g>", "123456789ABCDEFGHIJKLMNOPQRSTUVXZ123456789"));
        ...
        private void DefineDadosImpressaoCancelamento()
        {
            ...
            {               
                ...
                Linhas.Add(String.Format("<bc>Extrato No. {0}", dadosSAT.numeroCFe));
                Linhas.Add(String.Format("<bc>CUPOM FISCAL ELETRONICO - SAT"));
                Linhas.Add(String.Format("<bc>Cancelamento"));

                Linhas.Add(String.Format("<bc>DADOS DO CUPOM FISCAL CANCELADO"));
                Linhas.Add(String.Format("<bc>No. {0}", dadosCancelamento.numero));
                Linhas.Add(String.Format("<bc>----------------------------------------------------"));
                //-------
                ...
                Linhas.Add(String.Format("<nb>TOTAL:{0}", dadosCancelamento.valor));

                //outro XML                
                Linhas.Add(String.Format("<bc>SAT No. {0}", dadosCancelamento.serie));
                Linhas.Add(String.Format("<nc>{0} - {1}", dadosCancelamento.data, dadosCancelamento.hora));
                ...
                //sb.AppendLine(String.Format("<qr>{0}", dadosQRCancelamento));
                ...
                //Rodapé ---------------------------------------------------
                Linhas.Add(String.Format("<bc>DADOS DO CUPOM FISCAL DE CANCELAMENTO"));
                Linhas.Add(String.Format("<bc>SAT No. {0}", dadosSAT.numSerieSAT));
                Linhas.Add(String.Format("<nc>{0} - {1}", dadosSAT.dataEmis, dadosSAT.horaEmis));
        ...
        //        //sb.AppendLine(String.Format("<c><b><ce>------------------------------------------------</ce></c></b>"));
        //        Linhas.Add(String.Format("<cb>{0}</ce></c></b>", dadosEmit.nomeFantasia));
        //        Linhas.Add(String.Format("<cb>{0}</ce></c></b>", dadosEmit.razao));
        //        Linhas.Add(String.Format("<c><b><ce>Endereço:{0}, No:{1}</ce></b></c>", dadosEmit.logradouro, dadosEmit.numero));
        //        Linhas.Add(String.Format("<c><b>CNPJ:{0}     I.E.:{1}     I.M.: </b></c>", dadosEmit.CNPJ, dadosEmit.IE));

        //        Linhas.Add(String.Format("<c><b><ce>----------------------------------------------------</ce></b></c>"));

        //        Linhas.Add(String.Format("<n><b><ce>Extrato No. {0}</ce></b></n>", dadosSAT.numeroCFe));
        //        Linhas.Add(String.Format("<n><b><ce>CUPOM FISCAL ELETRONICO - SAT\nCancelamento</ce></b></n> <l></l>"));

        //        Linhas.Add(String.Format("<c><b><ce>DADOS DO CUPOM FISCAL CANCELADO</ce></b></c>"));
        //        Linhas.Add(String.Format("<c><b><ce>No. {0}</ce></b></c>", dadosCancelamento.numero));
        //        Linhas.Add(String.Format("<c><b><ce>----------------------------------------------------</ce></b></c>"));
        //        //-------
        ...
        //        if (!String.IsNullOrEmpty(dadosCli.cpf_cnpj_destinatario))
        //        {
        //            Linhas.Add(String.Format("<c>CPF-CNPJ Consumidor:    {0}</c>", dadosCli.cpf_cnpj_destinatario));
        //        }
        //        else
        //        {
        //            Linhas.Add(String.Format("<c>CPF-CNPJ Consumidor:    {0}</c>", "Nao Identificado"));
        //        }

        //        Linhas.Add(String.Format("<c><b>TOTAL:</b>{0}</c>", dadosCancelamento.valor));

        //        //outro XML                
        //        Linhas.Add(String.Format("<c><b><ce>SAT No. {0}</ce></c></b>", dadosCancelamento.serie));
        //        Linhas.Add(String.Format("<c><ce>{0} - {1}</ce></c>", dadosCancelamento.data, dadosCancelamento.hora));
        ...
        //        Linhas.Add(String.Format("<c><ce>{0}</ce></c><l></l>", chave));

        //        Linhas.Add(String.Format("<code128>{0}</code128><code128>{1}</code128>", tmpChave.Substring(0, 22), tmpChave.Substring(22, 22)));
        ...
        //        //Rodapé ---------------------------------------------------
        //        Linhas.Add(String.Format("<c><b><ce>DADOS DO CUPOM FISCAL DE CANCELAMENTO</ce></b></c>"));
        //        Linhas.Add(String.Format("<c><b><ce>SAT No. {0}</ce></c></b>", dadosSAT.numSerieSAT));
        //        Linhas.Add(String.Format("<c><ce>{0} - {1}</ce></c>", dadosSAT.dataEmis, dadosSAT.horaEmis));
